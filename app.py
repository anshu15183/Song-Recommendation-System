...
if st.button('Recommend',):
    ...
    with col3:
         if preview_url:
            st.markdown(f'<div style="{center_style}">👉</div></br></br>',unsafe_allow_html=True)st.markdown(f'<style>{keyframes}</style>'f'<a href="{preview_url}" target="_blank" style="{link_style}">🎵 Play Preview 🎵</a></br>',unsafe_allow_html=True)
